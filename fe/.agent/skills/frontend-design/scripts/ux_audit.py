...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_form = bool(re.search(r'<form|<input|password|credit[-_]?card|payment', content, re.IGNORECASE)) if not is_css else False
        ...
        is_navbar = 'navbar' in filename.lower()
        if nav_items > 7 and not is_css and not is_footer and not is_navbar:
        ...
        if len(font_families) > 3 and not is_css:
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'scratch', 'brain'}]
            for file in files:
