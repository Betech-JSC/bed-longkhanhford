...
## 🔴 CRITICAL FIGMA RULE (P0 - MANDATORY)

- **Do NOT change the design under any circumstances if a Figma design exists.**
- **TUYỆT ĐỐI không tự ý thay đổi hay sáng tạo thiết kế khác Figma nếu đã có thiết kế trên Figma.** Giữ độ chính xác 100% về khoảng cách (spacing, padding, margin, gap), bố cục (layout), cỡ chữ (font size, weight, line height), màu sắc (color tokens) và cấu trúc component.
- Self-deception, shortcuts, or "close enough" implementations are violations of quality control.
...
## 🚗 DN Ford Project Specific Guidelines

When working on the **DN Ford** codebase, you must adhere to the following rules:

### 1. Layout & Grid Standards
*   **Uniform Container**: Wrap all standard content in:
    ```tsx
    className="max-w-[1440px] mx-auto px-4 xl:px-[144px] w-full"
    ```
*   **Body & Section Backgrounds**: The default background for sections is `bg-gray-50` (`#F9FAFB`) unless specified as transparent or customized (e.g. Test Drive dark section).

### 2. Infinite Autoplay Sliders (Popular Fleet, Testimonials)
*   **Continuous Loop**: Implement continuous slider transition loops.
*   **Interaction Handlers**:
    *   Pause autoplay on `onMouseEnter` or when a user clicks/interacts with navigation controls.
    *   **Inactivity Recovery**: Use a timer hook (`useEffect` with cleanup) to automatically reset interaction states after **5 seconds** of inactivity, resuming autoplay.

### 3. Hero Banner Architecture
*   **Bottom Alignment**: Align the hero titles and copy to the bottom of the section.
*   **Figma-matching Overlay**: Use a smooth linear dark gradient at the bottom (`bg-gradient-to-t from-black/60 to-transparent`).
*   **Quick Tabs**: Keep bottom tabs flush with the bottom container edge (`pb-0`).

### 4. Floating Action Buttons (FAB) & Interactions
*   **Circular Buttons (Zalo, Phone)**: Size must be 56px (`w-14 h-14`) with hover scaling effect.
*   **Test Drive FAB**: Use the dark navy pill button shape aligned with Figma design specs.
*   **Social Link**: Hardcode the official Facebook Messenger and Page URL: `https://www.facebook.com/FordDongNai.Official`.
