...
export interface TinTucClientProps {
  initialData?: any;
  initialReviews?: any[];
}

function NewsListPageContent({ initialData, initialReviews }: { initialData?: any; initialReviews?: any[] }) {
  const [categories, setCategories] = useState<any[]>(initialData?.categories || []);
  const [activeTab, setActiveTab] = useState<number | "all">("all");
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState("");
  const [debouncedSearchQuery, setDebouncedSearchQuery] = useState("");
  const [posts, setPosts] = useState<any[]>(initialData?.posts?.data || []);
  const [topPosts, setTopPosts] = useState<any[]>(initialData?.top_posts || []);
  const [totalPages, setTotalPages] = useState(initialData?.posts?.last_page || 1);
            ...
            {topPosts.slice(0, 2).map((art) => (
              ...
              >
                ...
                <div className="aspect-[600/380] relative overflow-hidden w-full bg-white flex items-center justify-center p-4">
                  ...
                  />
                  {art.category && (
                    <div className="absolute top-4 left-4 bg-[#066fef] text-white text-xs font-semibold px-3 py-1 rounded-[4px] uppercase tracking-wider z-10">
                      {art.category.title}
                    </div>
                  )}
                </div>
...
export default function TinTucClient({ initialData, initialReviews }: TinTucClientProps) {
  return (
    <Suspense fallback={
      ...
      <NewsListPageContent initialData={initialData} initialReviews={initialReviews} />
    </Suspense>
  );
}
