...
// API imports removed — data is fetched server-side and passed via props
...
const INITIAL_BRAND_ITEMS = [
  ...
  },
  {
    ...
    image: "/assets/transit-hero.webp",
    link: "/lien-he"
  }
];
...
export default function HomeClient({
  ...
  const [heroSlides] = useState<any[]>(initialHeroSlides);
  const [homeArticles] = useState<any[]>(initialArticles);
  const [categories] = useState<any[]>(initialCategories);
  const [vehiclesList] = useState<any[]>(initialVehicles);
  const [brandItems] = useState<any[]>(initialBrandItems);
  const [servicesList] = useState<any[]>(initialServices);
  const [customerHandovers] = useState<any[]>(initialHandovers);
  const isVehiclesLoading = false; // Data already loaded from server
  ...
  // Data fetching removed — all data received via props from Server Component
  ...
  // Posts data fetching removed — data received via props from Server Component
          ...
          <div className="max-w-[1152px] mx-auto w-full px-6 xl:px-0 space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
              ...
              <div className="lg:col-span-7 relative h-[320px] sm:h-[400px] lg:h-auto min-h-[350px] w-full rounded-[12px] overflow-hidden border border-neutral-200/80 shadow-sm bg-gray-100">
                {[
                  { src: siteAssets.serviceMaintenance, pos: "object-top" },
                  { src: siteAssets.serviceDelivery, pos: "object-center" },
                  { src: siteAssets.serviceCustomerCare, pos: "object-top" }
                ].map((img, idx) => {
                  const isActive = activeServiceTab === idx;
                  return (
                    <div
