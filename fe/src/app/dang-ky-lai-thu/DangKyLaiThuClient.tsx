...
import { MapPin, Phone, Mail, CheckCircle, X, Car, Clock } from "lucide-react";
...
export interface DangKyLaiThuClientProps {
  initialVehicles?: any[];
}

function TestDriveFormContent({ initialVehicles }: { initialVehicles?: any[] }) {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 xl:px-[80px] py-6 w-full">
      {/* Breadcrumb Navigation */}
      <Breadcrumbs items={[{ label: "Đăng ký lái thử" }]} className="mb-6" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start mb-12">
        ...
        <div id="test-drive-form" className="bg-[#003478] flex flex-col gap-6 p-8 rounded-[16px] shadow-lg text-white scroll-mt-20">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {/* Ghi chú & Khung giờ chọn nhanh */}
            <div className="flex flex-col gap-1.5">
              <label className="font-['Ford_Antenna',sans-serif] font-medium text-sm text-white">
                Ghi chú & Khung giờ lái thử
              </label>
              <textarea
                value={formNote}
                onChange={(e) => setFormNote(e.target.value)}
                placeholder="Nhập thời gian rảnh mong muốn hoặc chọn khung giờ bên dưới..."
                className="w-full h-[80px] bg-white border border-[#d6d6d6] text-gray-900 placeholder-[#808080] rounded-[8px] px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#0562d2] focus:ring-4 focus:ring-[#0562d2]/20 transition shadow-sm resize-none font-sans"
              />
              <div className="flex flex-wrap gap-2 mt-1">
                {[
                  "Sáng: 08:00 - 11:30",
                  "Chiều: 13:30 - 17:00",
                  "Lái thử Cuối tuần (T7 / CN)"
                ].map((slot) => (
                  <button
                    key={slot}
                    type="button"
                    onClick={() => {
                      if (!formNote.includes(slot)) {
                        setFormNote((prev) => (prev ? `${prev} | ${slot}` : slot));
                      }
                    }}
                    className="text-[11px] font-medium bg-white/10 hover:bg-white/20 text-white px-2.5 py-1 rounded-full border border-white/20 transition cursor-pointer"
                  >
                    + {slot}
                  </button>
                ))}
...
export default function DangKyLaiThuClient({ initialVehicles }: DangKyLaiThuClientProps) {
  return (
    <div className="bg-[#fafafa] flex-1 min-h-screen">
      <Suspense fallback={
        ...
        <TestDriveFormContent initialVehicles={initialVehicles} />
      </Suspense>
    </div>
  );
}
