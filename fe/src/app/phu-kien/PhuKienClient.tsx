...
export interface PhuKienClientProps {
  initialAccessories?: any[];
  initialCategories?: any[];
}

export default function PhuKienClient({ initialAccessories, initialCategories: serverCategories }: PhuKienClientProps) {
  const [accessories, setAccessories] = useState<AccessoryItem[]>(initialAccessories || []);
  const [categories, setCategories] = useState<any[]>(staticCategories);
  const [isLoading, setIsLoading] = useState(!initialAccessories?.length);
